...
export class CommonApiService {
    ...
    statList:any=[];
    getStates(search?:any, country?:any) {
        this.api.post({ 'filters':{'search': search}, 'country':country}, 'postal-code/states').subscribe(result => {
            if (result.statusCode === 200) {
    ...
    getProduct(search?: any) {
        this.api.post({page : 1, 'dropdown-name': 'Product', filters: { 'search': search }}, 'product/read-dropdown').subscribe(result => {
